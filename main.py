...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    train_opp = optimizer.minimize(cross_entropy_loss)

    return logits, train_opp, cross_entropy_loss
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        ...
        for image, labels in get_batches_fn(batch_size):
            _, loss = sess.run([train_op, cross_entropy_loss], feed_dict={input_image: image, correct_label: labels, keep_prob: 0.8, learning_rate: 1e-4})
            losses.append(loss)
        print("Epoch: {0}/{1} Loss: {2:2f}".format(epoch + 1, epochs, loss))
